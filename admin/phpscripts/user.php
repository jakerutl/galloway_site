<?php
	...
	function editStudent($id, $fname, $username, $password, $email) {
		include('connect.php');
		...
		if($updatequery) {
			redirect_to("student_edituser.php");
		}else{
			$message = "Guess you got canned...";
			return $message;
		}
	...
	function deletePost($id) {
		include('connect.php');
		$delstring = "DELETE FROM tbl_post WHERE post_id = {$id}";
		$delquery = mysqli_query($link, $delstring);
		if($delquery) {
	...
	function deletestudentPost($id) {
		include('connect.php');
		$delstring = "DELETE FROM tbl_studentPost WHERE s_post_id = {$id}";
		$delquery = mysqli_query($link, $delstring);
		if($delquery) {
